...
describe("session.ts", () => {
  ...
  describe("seedSubagentSessionFile", () => {
    ...
    it("fork truncates from the LAST user message, keeping earlier full turns", () => {
      const secondUser = {
        type: "message",
        id: "user-002",
        parentId: "asst-001",
        message: { role: "user", content: [{ type: "text", text: "Now spawn a worker" }] },
      };
      const parentFile = createSessionFile(dir, [
        SESSION_HEADER,
        MODEL_CHANGE,
      ...
      const childFile = join(dir, "fork-truncate-child.jsonl");

      seedSubagentSessionFile({
        mode: "fork",
        parentSessionFile: parentFile,
        childSessionFile: childFile,
        childCwd: "/tmp/fork-truncate-cwd",
      });
      ...
      const entries = readFileSync(childFile, "utf8")
        ...
        .split("\n")
        .map((line) => JSON.parse(line));
      // header + model_change + first user turn + first assistant turn;
      // the last user message and everything after it are dropped.
      assert.deepEqual(
        entries.map((e) => e.id).slice(1),
        ["mc-001", "user-001", "asst-001"],
      );
      assert.equal(entries.some((e) => e.id === "user-002"), false);
      assert.equal(entries.some((e) => e.id === "asst-002"), false);
    });
  });
